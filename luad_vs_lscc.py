...
def _load_tumor_rna(subtype: str, data_dir: Path, dataset_split: str) -> pd.DataFrame:
    path = data_dir / f"{subtype}_{dataset_split}_rna_expression_tumor.tsv"
...
def load_dataset(
    ...
    luad = _load_tumor_rna("LUAD", data_dir, dataset_split)
    lscc = _load_tumor_rna("LSCC", data_dir, dataset_split)

    x = pd.concat([luad, lscc], axis=0).fillna(NAN_FILL_VALUE)
    y = pd.Series(
        [0] * len(luad) + [1] * len(lscc),
        index=x.index,
        name="label",
        dtype=int,
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train/evaluate the LUAD-vs-LSCC classifier.")
